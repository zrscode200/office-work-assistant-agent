...
2. Start the dashboard server in the background and capture the URL:
   ...
   ```bash
   node .codex/dashboard/server.js &
   sleep 0.5
...
- If the server fails to start, check that `.codex/dashboard/server.js` and `.codex/dashboard/template.html` exist. If missing, the workspace may need an update (`bootstrap/init-workspace.sh --update`).
