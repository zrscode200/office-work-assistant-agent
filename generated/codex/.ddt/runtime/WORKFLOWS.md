...
| `catch-up` | `since` ISO timestamp, optional `summary` | Records changed after that time, each with `change: {authored, arrived, jira_refreshed}`: saved locally, arrived through a pull (from the clone's reflog), or refreshed from Jira. Commits made under this clone's own Git identity are the user's publications and count as authored, not arrived. When a clone's reflog is unavailable, a warning says arrivals for that scope are unknown and only author times are used |
...
1. `sync-status` with `scope` shows local HEAD, branch, pending changes, and last-known upstream counts. It does not contact the remote. For a clone nested in the workspace it adds `nested` with the relative path and whether the workspace repository ignores it, and points out a clone that was already recorded there as a pointer.
...
6. A failed push leaves the commit in place and reports `published:false` with a redacted `cause`. If the remote moved (`error` mentions `sync-fetch`): run `sync-fetch`, then `sync-rebase` with `scope`, exact `expected_commit`, the reviewed `destination`, and `confirm:true`. It replays the single outgoing project-only commit onto the fetched upstream with Git plumbing, like `publish`: no hooks or editors run and the original author and message are preserved. Commits created by `publish` and `sync-rebase` are not GPG-signed; a branch rule that requires signed commits rejects them. Records changed on both sides are merged field by field. A field changed differently on both sides, a record both clones created with differing fields, an identity field (`id`, `provider`, `jira`, `legacy_source`) that differs, or a record upstream removed or moved is returned under `unresolved` with `local` and `upstream` versions; nothing is written or rewritten. Rerun with `resolution: {"<path>": {"<field>": "local" | "upstream" | {"value": ...}}}` for the listed non-identity fields after agreeing the merged meaning. Choices for fields that did not conflict are ignored, and a resolved record must pass the same validation as a save (for example, no private source paths in shared records). Then `sync-push` with the new `head` as `expected_commit`. `sync-push` allows at most one outgoing project-only commit and pushes only that commit to that explicit branch. Never force push or silently rewrite history. A local save or commit is not evidence that the team received it.
