...
This workspace is powered by Codex and organized around one idea: talk
...
4. Open Codex in this directory.
5. Ask for the `self-tutorial` reference for a guided walkthrough.

## How Skill Routing Works

Codex discovers three workspace skills through `.codex/config.toml`:

| Skill | Activates when you say... | Uses references for |
|-------|--------------------------|---------------------|
...
The behavior details live as reference files under
`.codex/skills/*/references/`. You usually do not need to name a reference
directly. Describe the work naturally and the matching skill should use the
right reference.
...
To open it, ask Codex to open the dashboard. The project-manager skill uses the
`dashboard` reference, starts the local server, and reports the URL.
