...
- Collaboration fixtures use two disposable local clones and a local bare remote. They verify exact-byte publication, explicit destinations, staged/outgoing exclusions, propagation, failed-push retry, arrival-aware catch-up, pull tolerance of untracked files, and fetch/rebase recovery with field-level record merging and explicit conflict resolution. Helper Git calls run with a neutral global configuration. No production repository is used.
